...
const downloadExcel = () => {
  ...
  transformedDomains.value.forEach((domain) => {
    const domainStartRow = currentRow; // Domain 병합 시작 위치
    domain.principles.forEach((principle) => {
      const principleStartRow = currentRow; // Principle 병합 시작 위치
      principle.standards.forEach((standard) => {
        const standardStartRow = currentRow; // Standard 병합 시작 위치
        standard.evaluations.forEach((evaluation) => {
          const evaluationScore =
            evaluationScores.value[evaluation.evaluationCode] || "평가중"; // 올바른 매핑
          const row = [
            domain.domainName, // Domain
            principle.principleName, // Principle
            standard.standardName, // Standard
            evaluation.evaluationCode, // 세부평가코드
            evaluation.evaluationName, // 세부평가사항
            evaluationScore, // 세부평가점수
            calculateStandardAverage(standard.evaluations) || "평가중", // Standard 평균
            calculatePrincipleAverage(principle.standards) || "평가중", // Principle 평균
            calculateDomainAverage(domain.principles) || "평가중", // Domain 평균
          ];
        ...
        if (standard.evaluations.length > 1) {
          merges.push({
            s: { r: standardStartRow, c: 2 }, // Standard 이름 시작 셀
            e: { r: currentRow - 1, c: 2 }, // Standard 이름 끝 셀
          });
          merges.push({
            s: { r: standardStartRow, c: 6 }, // Standard 평균 시작 셀
            e: { r: currentRow - 1, c: 6 }, // Standard 평균 끝 셀
          });
        }
      });
      ...
      if (currentRow - principleStartRow > 1) {
        merges.push({
          s: { r: principleStartRow, c: 1 }, // Principle 이름 시작 셀
          e: { r: currentRow - 1, c: 1 }, // Principle 이름 끝 셀
        });
        merges.push({
          s: { r: principleStartRow, c: 7 }, // Principle 평균 시작 셀
          e: { r: currentRow - 1, c: 7 }, // Principle 평균 끝 셀
        });
      }
    });
    ...
    if (currentRow - domainStartRow > 1) {
      merges.push({
        s: { r: domainStartRow, c: 0 }, // Domain 이름 시작 셀
        e: { r: currentRow - 1, c: 0 }, // Domain 이름 끝 셀
      });
      merges.push({
        s: { r: domainStartRow, c: 8 }, // Domain 평균 시작 셀
        e: { r: currentRow - 1, c: 8 }, // Domain 평균 끝 셀
      });
    }
  });
